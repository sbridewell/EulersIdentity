...
- The UI must follow the MVVM pattern, and prefer data binding over code-behind.
- UI components must be optimised for ease of unit testing.

# Testing

- Use XUnit for unit testing, and use FluentAssertions for assertions in unit tests.
- Unit tests (EulersIdentity.Test and EulersIdentity.WPF.Test) must follow the Arrange, Act, Assert pattern.
...
- Use FlaUI and LightBDD for UI testing.
- UI tests must use the Given, When, Then pattern.
